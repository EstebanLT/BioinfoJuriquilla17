...
require(PharmPlotter)
library(PharmPlotter,devtools,ggplot)
...
```{r Q1}
#Finding out where the cell lines are stored in the data set
head(exerciseData)
#Finding different cell lines
difCL = unique(exerciseData$cellLine)
head(difCL)
#Counting them
length(difCL)
...
```{r Q2}
#Selecting unique concentrations from each study
concCCLE = unique(subset(exerciseData, study == "CCLE", select = concentration))
sort(concCCLE$concentration)
concGDSC = unique(subset(exerciseData, study == "GDSC", select = concentration))
sort(concGDSC$concentration)
...
```{r Q3}
#Making a histogram for each study
ggplot(data = exerciseData, aes(viability)) + geom_histogram() + facet_wrap(~study)
...
```{r Q4}
#Calculating the total number of observations with viability between 0 and 100
nrow(exerciseData[exerciseData$viability >= 0 & exerciseData$viability <= 100,])
#Calculating the number of observations with viability between 0 and 100 for each drug
sum(tapply( exerciseData$viability, exerciseData$drug, function(x){
        length( x[x >= 0 & x <= 100] )
    }))
#An alternative to do the same
#First selecting the subset of all data whith viability from 0 to 100
Caca = exerciseData[exerciseData$viability >= 0 & exerciseData$viability <= 100,]
#Then calculating the total by drug
sum(aggregate(Caca$viability,by = list(Caca$drug),FUN = length)$x)
...
ggplot(exerciseData, aes(viability)) + geom_histogram() + facet_wrap(~doseID)
